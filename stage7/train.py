def training(reviews, univocabulary, bivocabulary):
    ...
    for word in bivocabulary:
        ...
        if positiveReview.count(word) == 0:
            ...
            positive_bi_Probabilities.append(
                (1.0 * positiveCount)/positiveReview.count(w1))
        ...
        if negativeReview.count(word) == 0:
            ...
            negative_bi_Probabilities.append(
                (1.0 * negativeCount)/negativeReview.count(w1))

    for i in range(len(univocabulary)):
        # (count(word in +) + 1) / sum of (count(word' in +) + 1)
        positive_Uni_Probabilities.append(
            (1.0 * positive_uni_counts[i])/totalCountInPositive)
        # (count(word in -) + 1) / sum of (count(word' in -) + 1)
        negative_Uni_Probabilities.append(
            (1.0 * negative_uni_counts[i])/totalCountInNegative)
