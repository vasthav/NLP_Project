def training(reviews, vocabulary):
    ...
    for word in vocabulary:
        ...
        positiveProbabilities.append(
            (1.0 * positiveCount)/totalCountInPositive)

        negativeCount = negativeReview.count(word) + 1
        # (count(word in -) + 1) / sum of (count(word' in -) + 1)
        negativeProbabilities.append(
            (1.0 * negativeCount)/totalCountInNegative)
